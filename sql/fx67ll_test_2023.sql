...
CREATE TABLE `fx67ll_ai_prompt_template` (
  ...
  `prompt_status` char(1) NOT NULL DEFAULT '0' COMMENT '模板启用状态（字典码：0-启用，1-停用）',
  `prompt_remark` varchar(1023) DEFAULT '' COMMENT '模板业务备注（说明使用场景、注意事项等）',
  `user_id` bigint(20) NOT NULL COMMENT '用户ID',
  `create_by` varchar(64) DEFAULT '' COMMENT '记录创建者用户名',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '记录创建时间',
  `update_by` varchar(64) DEFAULT '' COMMENT '记录最后更新者用户名',
...
CREATE TABLE `fx67ll_ai_prompt_group` (
  ...
  `group_status` char(1) NOT NULL DEFAULT '0' COMMENT '分组启用状态（字典码：0-启用，1-停用）',
  `group_sort` int(4) DEFAULT 0 COMMENT '分组展示排序（升序排列，数值越小越靠前）',
  `group_remark` varchar(1023) DEFAULT '' COMMENT '分组业务备注（说明分组的用途、范围）',
  `user_id` bigint(20) NOT NULL COMMENT '用户ID',
  `create_by` varchar(64) DEFAULT '' COMMENT '记录创建者用户名',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '记录创建时间',
  `update_by` varchar(64) DEFAULT '' COMMENT '记录最后更新者用户名',
...
CREATE TABLE `fx67ll_ai_prompt_scene` (
  ...
  `scene_desc` varchar(1023) NOT NULL COMMENT '场景业务描述（说明场景的业务背景、应用范围）',
  `scene_status` char(1) NOT NULL DEFAULT '0' COMMENT '场景启用状态（字典码：0-启用，1-停用）',
...
CREATE TABLE `fx67ll_ai_prompt_model` (
  ...
  `model_status` char(1) NOT NULL DEFAULT '0' COMMENT '模型启用状态（字典码：0-启用，1-停用）',
...
CREATE TABLE `fx67ll_ai_request_limit_rule` (
  ...
  `flow_control_mode` char(1) DEFAULT '' COMMENT '流控模式（字典码：D-直接拒绝，A-关联控制，L-链路流控，仅流控规则有效）',
  `flow_control_effect` char(1) DEFAULT '' COMMENT '流控效果（字典码：F-快速失败，W-预热启动，Q-匀速排队，仅流控规则有效）',
  `flow_rule_type` char(1) DEFAULT '' COMMENT '流控指标类型（字典码：Q-QPS阈值，C-并发线程数，仅流控规则有效）',
  `flow_threshold` decimal(10,2) DEFAULT 0 COMMENT '流控阈值（QPS或并发数，保留2位小数）',
  `circuit_strategy` char(1) DEFAULT '' COMMENT '熔断策略（字典码：S-慢调用比例，E-异常比例，N-异常数，仅熔断规则有效）',
  `circuit_threshold` decimal(10,2) DEFAULT 0 COMMENT '熔断触发阈值（慢调用/异常比例：0-1；异常数：正整数）',
  `circuit_grade` int(4) DEFAULT 0 COMMENT '慢调用判定阈值（毫秒，仅慢调用熔断策略有效）',
  `circuit_window` int(4) DEFAULT 0 COMMENT '熔断统计窗口时长（毫秒，默认10秒）',
  `circuit_timeout` int(4) DEFAULT 0 COMMENT '熔断恢复超时时间（毫秒，默认5秒后尝试半开）',
  `limit_rule_status` char(1) DEFAULT '0' COMMENT '规则启用状态（字典码：0-启用，1-停用）',
  `limit_rule_remark` varchar(1023) DEFAULT '' COMMENT '规则备注',
  `user_id` bigint(20) NOT NULL COMMENT '用户ID',
  `create_by` varchar(64) DEFAULT '' COMMENT '记录创建者用户名',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '记录创建时间',
  `update_by` varchar(64) DEFAULT '' COMMENT '记录最后更新者用户名',
...
) ENGINE=innodb DEFAULT CHARSET=utf8mb4 COMMENT='AI 调用请求限流/熔断规则表（适配Sentinel框架）';
...
CREATE TABLE `fx67ll_ai_request_log` (
  ...
  `cost_currency` varchar(10) DEFAULT 'CNY' COMMENT '计价货币类型（ISO 4217货币码，如CNY、USD）',
  ...
  `create_by` varchar(64) DEFAULT '' COMMENT '调用用户名',
...
CREATE TABLE `fx67ll_dortmund_season` (
  ...
  `season_status` char(1) NOT NULL DEFAULT '0' COMMENT '赛季状态（字典码：0-进行中，1-已结束，2-未开始）',
  `season_sort` int(4) DEFAULT 0 COMMENT '赛季展示排序（升序排列，数值越小越靠前）',
  `season_remark` varchar(1023) DEFAULT '' COMMENT '赛季业务备注（说明赛事级别、规则等）',
  `user_id` bigint(20) NOT NULL COMMENT '用户ID',
  `create_by` varchar(64) DEFAULT '' COMMENT '记录创建者用户名',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '记录创建时间',
  `update_by` varchar(64) DEFAULT '' COMMENT '记录最后更新者用户名用户名',
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '记录最后更新时间',
  `del_flag` char(1) DEFAULT '0' COMMENT '逻辑删除标志（字典码：0-存在，2-已删除）',
